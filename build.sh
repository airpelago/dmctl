...
for PLATFORM in $PLATFORMS; do
  ...
  CMD="GOOS=${GOOS} GOARCH=${GOARCH} go build -o bin/${BIN_FILENAME} $@"
  echo "${CMD}"
...
#  CMD="GOOS=linux GOARCH=arm64 go build -o bin/${OUTPUT}-linux-arm64 $@"
#  echo "${CMD}"
#  eval $CMD || FAILURES="${FAILURES} ${PLATFORM}"
#fi
for GOOS in $PLATFORMS_ARM; do
  ...
  for GOARM in 7; do
    ...
    CMD="GOARM=${GOARM} GOOS=${GOOS} GOARCH=${GOARCH} go build -o bin/${BIN_FILENAME} $@"
    echo "${CMD}"
